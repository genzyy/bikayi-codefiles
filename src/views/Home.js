...
const Home = () => {
    ...
    return (
        <>
                        <Filter categories={categories} filterCat={filterCat} filterYear={filterYear} setFilterCat={setFilterCat}  setFilterYear={setFilterYear}/>
            <div className='grid grid-cols-3 gap-3 bg-gray'>
                {
                    data.prizes.map((p, index) => {
                                       if(filterCat === 'all' && filterYear === 'all' && categories.includes(p.category)) {
                                            return (
                                            <div key={index} className='w-full px-4 py-2 m-5 shadow-lg rounded-lg bg-white'>
                                            ...
                                            <p className='text-md font-medium capitalize'>The Awardees:</p>
                                                <div className='my-3'>
                                                    {
                                            p.laureates && p.laureates.map((u, uindex) => (
                                                    <p key={uindex} className='italic capitalize text-sm font-medium pl-3'>{`${uindex + 1}. ${u.firstname} ${u.surname}`}</p>
                                                ))
}
                                                </div>
                                        </div>
